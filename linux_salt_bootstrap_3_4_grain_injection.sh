...
function install_salt_repo() {
  rpm --import https://archive.repo.saltstack.com/yum/redhat/7/x86_64/archive/2016.3.4/SALTSTACK-GPG-KEY.pub
  ...
  echo 'baseurl=https://archive.repo.saltstack.com/yum/redhat/$releasever/$basearch/archive/2016.3.4' >> $repofile
  echo 'enabled=1'  >> $repofile
  echo 'gpgcheck=1' >> $repofile
  echo 'gpgkey=https://archive.repo.saltstack.com/yum/redhat/$releasever/$basearch/archive/2016.3.4/$releaseverSALTSTACK-GPG-KEY.pub' >> $repofile
  yum clean all
}
...
function install_salt_minion() {
  ...
  echo "region: $region" > /etc/salt/grains
